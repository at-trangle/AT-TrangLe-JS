...
function list_video() {
  for(let i = 0; i < listNum; i++) {
    if(playing['url'] !== videosList[i]['url']) {
      $('#video-list').append('<li class = "video-item"><button id= "'+videosList[i]['id']+'" onclick = "changeVideo(this.id)" class = "change-btn"><iframe src="' + videosList[i]['url']+'" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe></button></li>')
    }
  }
}
